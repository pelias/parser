...
class MustNotPreceedFilter {
  ...
  solve(tokenizer) {
    tokenizer.solution = tokenizer.solution.filter((s) => {
      let object = s.pair.filter(
        (p) => p.classification.constructor.name === this.classification.object,
      );
      let subject = s.pair.filter(
        (p) =>
          p.classification.constructor.name === this.classification.subject,
      );
      ...
      if (object.length > 0 && subject.length > 0) {
        ...
        if (subject.some((p) => p.span.start > object[0].span.end)) {
          ...
          s.pair = s.pair.filter(
            (p) =>
              p.classification.constructor.name !== this.classification.object,
          );
