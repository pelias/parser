...
class HouseNumberPositionPenalty extends BaseSolver {
  solve(tokenizer) {
    tokenizer.solution.forEach((s) => {
      const housenumber = s.pair.find(
        (p) => p.classification.constructor === HouseNumberClassification,
      );
      const street = s.pair.find(
        (p) => p.classification.constructor === StreetClassification,
      );
      ...
      if (
        !housenumber ||
        !street ||
        !street.classification.meta ||
        !street.classification.meta.langs
      ) {
        return;
      }
      ...
      if (
        numberLastLangs.hasOwnProperty(lang) &&
        housenumber.span.start < street.span.start
      ) {
        s.penalty += numberLastLangs[lang];
      } else if (
        numberFirstLangs.hasOwnProperty(lang) &&
        street.span.start < housenumber.span.start
      ) {
