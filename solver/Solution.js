class Solution {
  ...
  covers(solution) {
    return solution.pair.every((p) =>
      this.pair.some((pp) => pp.span.covers(p.span)),
    );
  }
  ...
  coversSameClassification(solution) {
    return solution.pair.every((p) =>
      this.pair.some((pp) => {
        return (
          pp.classification.constructor.name ===
            p.classification.constructor.name && pp.span.covers(p.span)
        );
      }),
    );
  }
  ...
  computeScore(tokenizer) {
    ...
    this.score =
      (score.confidence / score.coverage) *
      (score.coverage / tokenizer.coverage) *
      (1.0 - this.penalty);
  }
  ...
  mask(tokenizer) {
    ...
    for (let i = 0; i < body.length; i++) {
      ...
      let coveredBy = this.pair.filter(
        (p) => p.span.start <= i && p.span.end >= i,
      );
