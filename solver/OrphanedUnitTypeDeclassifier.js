...
class OrphanedUnitTypeDeclassifier {
  solve(tokenizer) {
    tokenizer.solution = tokenizer.solution.filter((s) => {
      ...
      let unitType = s.pair.filter(
        (p) => p.classification.constructor.name === 'UnitTypeClassification',
      );
      ...
      let unit = s.pair.filter(
        (p) => p.classification.constructor.name === 'UnitClassification',
      );
