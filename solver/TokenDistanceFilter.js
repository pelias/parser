...
class TokenDistanceFilter {
  solve(tokenizer) {
    tokenizer.solution = tokenizer.solution.filter((s) => {
      let housenumber = s.pair.filter(
        (p) =>
          p.classification.constructor.name === 'HouseNumberClassification',
      );
      let street = s.pair.filter(
        (p) => p.classification.constructor.name === 'StreetClassification',
      );
