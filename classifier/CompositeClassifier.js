...
class CompositeClassifier extends SectionClassifier {
  ...
  match(scheme, phrase) {
    ...
    if (!scheme.is.some((cl) => phrase.classifications.hasOwnProperty(cl))) {
      ...
      if (
        !scheme.is.some((cl) => children[0].classifications.hasOwnProperty(cl))
      ) {
        return false;
      }
    }
    ...
    if (children.length === 1) {
      if (
        scheme.not.some((cl) => children[0].classifications.hasOwnProperty(cl))
      ) {
        return false;
      }
    }
  ...
  each(section) {
    ...
    this.schemes.forEach((s) => {
      ...
      candidates = s.scheme.map((s) =>
        phrases.filter(this.match.bind(null, s)),
      );
        ...
        for (let i = 0; i < c.length; i++) {
          ...
          if (
            ...
            !curr.graph
              ...
              .graph.some(
                'next',
                (s) => s === next.graph.findOne('child:first'),
              )
          ) {
            return false;
          } else if (
            ...
            !curr.graph
              .findOne('child:first')
              .graph.some('prev', (s) => s === prev.graph.findOne('child:last'))
          ) {
            return false;
          }
          ...
          if (
            next &&
            next.classifications.hasOwnProperty('StreetClassification')
          ) {
            let firstChild = next.graph.findOne('child');
            if (
              firstChild &&
              firstChild.classifications.hasOwnProperty(
      ...
      if (typeof s.Class === 'function') {
        ...
        composites.forEach((c) => {
          ...
          superPhrases = superPhrases.concat(
            phrases.filter((p) => p.start === start && p.end === end),
          );
        });
