...
module.exports.tests.classify = (test) => {
  ...
  let fooHouseNum = new Span('1', 4).classify(
    new HouseNumberClassification(1.0),
  );
  foo.graph.add('next', fooHouseNum);

  let bar = new Span('Bar');
  let barHouseNum = new Span('2137', 4).classify(
    new HouseNumberClassification(1.0),
  );
  bar.graph.add('next', barHouseNum);

  let baz = new Span('Baz');
  let bazHouseNum0 = new Span('152/160', 4).classify(
    new HouseNumberClassification(1.0),
  );
  let bazHouseNum1 = new Span('152', 4).classify(
    new HouseNumberClassification(1.0),
  );
  let bazHouseNum2 = new Span('160', 8).classify(
    new HouseNumberClassification(1.0),
  );
  ...
  let quxHouseNum = new Span('1', 4).classify(
    new HouseNumberClassification(1.0),
  );
