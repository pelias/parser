...
class UnitTypeClassifier extends BaseClassifier {
  ...
  each(span, section) {
    ...
    for (let token in this.index) {
      ...
      if (
        span.norm.substring(0, token.length) === token &&
        /^\d+$/.test(span.norm.substring(token.length))
      ) {
        ...
        span.graph
          .findAll('prev')
          .forEach((prev) => unitType.graph.add('prev', prev));
        span.graph
          .findAll('next')
          .forEach((next) => unit.graph.add('next', next));
