...
const placetypes = {
  ...
  country: {
    files: [
      'name:*_x_preferred.txt',
      'wof:country.txt',
      'wof:country_alpha3.txt',
    ],
...
class WhosOnFirstClassifier extends PhraseClassifier {
  setup() {
    ...
    Object.keys(placetypes).forEach((placetype) => {
      ...
      whosonfirst.load(
        this.tokens[placetype],
        [placetype],
        placetypes[placetype].files,
        {
          minlength: 2,
      ...
      if (this.tokens[placetype].has(normalizedSpan)) {
        ...
        if (
          ...
          (span.graph.length('child') > 0 &&
            span.graph
              .findOne('child')
              .classifications.hasOwnProperty('StopWordClassification'))
        ) {
          return;
        }

        // classify tokens
        placetypes[placetype].classifications.forEach((Class) =>
          span.classify(new Class(confidence)),
        );
      }
    });
  }
