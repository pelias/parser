...
class Tokenizer {
  ...
  split() {
    for (let i = 0; i < this.section.length; i++) {
      this.section[i].setChildren(
        split(this.section[i], funcs.fieldsFuncWhiteSpace),
      );
      this.section[i].setChildren(
        split(this.section[i], funcs.fieldsFuncHyphenOrWhiteSpace),
      );
    }
  }
  ...
  computeCoverageRec(sum, curr) {
    ...
    return this.computeCoverageRec(
      sum + curr.end - curr.start,
      curr.graph.findOne('next'),
    );
  }
