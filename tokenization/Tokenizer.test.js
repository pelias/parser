...
module.exports.tests.split = (test) => {
  test('split: basic', (t) => {
    ...
    t.true(
      tok.section.every((s) =>
        s.graph.findAll('child').every((c) => c.constructor.name === 'Span'),
      ),
    );
...
module.exports.tests.permute = (test) => {
  test('permute: basic', (t) => {
    ...
    t.true(
      tok.section.every((s) =>
        s.graph.findAll('phrase').every((p) => p.constructor.name === 'Span'),
      ),
    );
  ...
  test('permute: advanced', (t) => {
    ...
    t.equals(
      tok.section[0].graph.findAll('phrase')[0].body,
    ...
    t.equals(
      tok.section[0].graph.findAll('phrase')[4].body,
      'West 26th Street',
    );
